...
class DSPPModule(nn.Layer):
    """
    Dense Spatial Pyramid Pooling.

    Args:
        dspp_ratios (tuple): The dilation rate using in ASSP module.
        in_channels (int): The number of input channels.
        out_channels (int): The number of output channels.
        align_corners (bool): An argument of F.interpolate. It should be set to False when the output size of feature
            is even, e.g. 1024x512, otherwise it is True, e.g. 769x769.
    ...
    def __init__(self,
        ...
        self.align_corners = align_corners
        self.data_format = data_format
        self.dspp_blocks = nn.LayerList()

        for ratio in dspp_ratios:
            if use_sep_conv:
                conv_func = layers.SeparableConvBNReLU
            else:
                conv_func = layers.ConvBNReLU
...
class SEPPModule(nn.Layer):
    ...
    def __init__(self,
        ...
        self.align_corners = align_corners
        self.data_format = data_format
        self.sepp_blocks = nn.LayerList()

        self.sepp1 = layers.ConvBN(
            in_channels=in_channels,
            out_channels=mid_channels,
            kernel_size=1,
        )
        self.cse = layers.cSE(mid_channels, 16)
        ...
        self.sepp2 = conv_func(
            in_channels=mid_channels,
            out_channels=mid_channels,
            kernel_size=3,
            dilation=sepp_ratios[0],
            padding=sepp_ratios[0],
            data_format=data_format,
        )
        self.sepp3 = conv_func(
            in_channels=mid_channels,
            out_channels=mid_channels,
            kernel_size=3,
            dilation=sepp_ratios[1],
            padding=sepp_ratios[1],
            data_format=data_format,
        )
        self.sepp4 = conv_func(
            in_channels=mid_channels,
            out_channels=mid_channels,
            kernel_size=3,
            dilation=sepp_ratios[2],
            padding=sepp_ratios[2],
            data_format=data_format,
        )
        self.conv = layers.ConvBN(
    ...
    def forward(self, x):
        ...
        x3 = self.sepp3(y)
        x4 = self.sepp4(y)
        x = paddle.concat((x1, x2, x3, x4), axis=axis)
        x = self.dropout(x)
        x = self.conv(x)
